...
inline int Move(char pos)
{
     ...
     st.depth++;  //Increasing depth of the state
...
int IterativeDeepening()
{
    ...
    t++;
    st.depth++;   //Increasing depth before recursively calling the function.
                  //We didn't land up in the above if(st==Goal) condition means we haven't achieved the goal yet.
    IterativeDeepening();
}
...
int main()
{
    ...
    Initial.depth=0; //Starting from depth zero.
